...
namespace WindowsFormsApp1.Apresentacao
{
    public partial class CRUD : Form
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string consultaSQL = "SELECT * FROM User013";
            ...
            try
            {
                ...
                @"\SQLEXPRESS;Initial Catalog=" + DataBase + ";Integrated Security=true"))
                {
                    ...
                    // Percorra as linhas do DataGridView
                    foreach (DataGridViewRow linha in BDSQL.Rows)
                    {
                        // Verifique se a linha não é a linha de novas inserções e se a célula foi editada
                        if (!linha.IsNewRow && linha.Cells["ID"].Value != null)
                        {
                            int id = Convert.ToInt32(linha.Cells["ID"].Value); // Suponha que "ID" é a coluna que identifica o registro
                            string novoValor = linha.Cells["Nome"].Value.ToString(); // Substitua "Nome" pelo nome da coluna que deseja atualizar

                            // Crie um comando SQL UPDATE para atualizar o registro no banco de dados
                            string consultaSQL1= "UPDATE User013 SET Nome = @NovoValor WHERE ID = @ID";
        ...
        {
            // Abre o Form1 para inserir novos dados
            Cadastro form1 = new Cadastro();
            form1.ShowDialog(); // Aguarda o fechamento do Form1

            // Verifique se o Form1 está fechado
            if (form1.IsDisposed)
            {
                // Verifique se o Form2 está aberto e feche-o
                Login form2 = Application.OpenForms["Form2"] as Login;
                if (form2 != null)
                {
                    form2.Close();
                }

                // Volte para o Form3 (este formulário)
                this.Show();
            }
        }
                ...
                @"\SQLEXPRESS;Initial Catalog=" + DataBase + ";Integrated Security=true"))
                {
                    ...
                    foreach (DataGridViewRow linha in BDSQL.Rows)
                    {
                        if (!linha.IsNewRow) // Evite a última linha em branco, se houver
                        {
                            ...
                            string consultaSQL = "UPDATE User013 SET Nome = @NovoValor WHERE ID = @ID";
                            using (SqlCommand cmd = new SqlCommand(consultaSQL, conexao))
                            {
                                cmd.Parameters.AddWithValue("@NovoValor", novoValor);
                                cmd.Parameters.AddWithValue("@ID", id);
                ...
                if (resultado == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        @"\SQLEXPRESS;Initial Catalog=" + DataBase + ";Integrated Security=true"))
                        {
                            ...
                            string consultaSQL = "DELETE FROM User013 WHERE ID = @ID";
                            using (SqlCommand cmd = new SqlCommand(consultaSQL, conexao))
                            {
                                cmd.Parameters.AddWithValue("@ID", id);
                                cmd.ExecuteNonQuery();
        ...
        private void txbpesq_Click(object sender, EventArgs e)
        {
            ...
            // Consulta SQL para pesquisar registros com base no nome, CPF ou email
            string consultaSQL = "SELECT * FROM User013 WHERE Nome LIKE @termo OR CPF LIKE @termo OR Email LIKE @termo";
        ...
        {
            // Criar uma instância do formulário de materiais
            Materiais formMateriais = new Materiais();

            // Exibir o formulário de materiais
            formMateriais.Show();
        }
